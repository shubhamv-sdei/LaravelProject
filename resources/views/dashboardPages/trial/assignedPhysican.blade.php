	                        ...
	                        <table class="table table-rounded assigned_inv_trial">
	                            ...
	                            <tbody class="align-middle">
	                                ...
	                                <tr>
	                                    ...
	                                    <td>{{isset($value['getAssignment']->physican_id) ? App\User::find($value['getAssignment']->physican_id)->getFullNameAttribute() : 'N/A'}}</td>
	                                    <td>
	                                    	@if(isset($value['getAssignment']->physican_status) && $value['getAssignment']->physican_status == '1')
                                			<span class="badge badge-primary p-2 w-100">Pending</span>
                                			@elseif(isset($value['getAssignment']->physican_status) && $value['getAssignment']->physican_status == '2')
                                			<span class="badge badge-success p-2 w-100">Approved</span>
                                			@elseif(isset($value['getAssignment']->physican_status) && $value['getAssignment']->physican_status == '3')
	                                    ...
	                                    <td class="action">
	                                    	...
	                                    	<button type="button" class="btn">
	                                    		<a class="btn" href="{{url('/dashboard/ViewSavedTrial')}}/{{$value->id}}"><i class="material-icons-outlined text-secondary">visibility</i></.000000000a>
	                                    	</button>
	                                    	...
	                                    	@if(isset($value['getAssignment']->physican_id) && $value['getAssignment']->physican_id)
	                                    	<button type="button" class="btn">
	                                    		<a href="{{url('/dashboard/startchat/')}}/{{(isset($value['getAssignment']->physican_id) ? $value['getAssignment']->physican_id : 'null')}}"
		...
		<div class="modal-dialog modal-dialog-centered modal-md">
			<div class="modal-content">
				...
				<div class="modal-header align-items-center">
					<h5 class="modal-title mb-0 text-secondary">Assign Physican</h5>
					<button type="button" class="close" data-dismiss="modal">&times;</button>
				</div>
				<!-- Modal body -->
				<div class="modal-body">
					<form class="form" action="{{route('Dashboard.assignPhysicanProcess')}}" method="POST">
						@csrf
						<div class="row gutters-2">
							<div class="col-md-12 form-group selectInvBlock">
								<label>Select Physican</label>
